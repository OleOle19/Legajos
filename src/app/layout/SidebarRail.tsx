...
export default function SidebarRail(props: SidebarRailProps) {
  return (
    <aside class="relative flex flex-col gap-4 overflow-hidden rounded-shell border border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(86,193,207,0.28),transparent_28%),radial-gradient(circle_at_bottom_left,rgba(213,140,60,0.28),transparent_24%),linear-gradient(180deg,rgba(18,37,61,1)_0%,rgba(22,52,91,1)_52%,rgba(16,31,50,1)_100%)] px-4 py-5 text-white shadow-shell xl:sticky xl:top-4 xl:max-h-[calc(100vh-2rem)] xl:overflow-auto">
      <div class="grid gap-4 xl:grid-cols-1">
        <div>
          <p class="text-[11px] uppercase tracking-[0.22em] text-white/78">Archivo de RR. HH.</p>
          <h1 class="mt-3 font-display text-[2rem] font-semibold leading-none tracking-[-0.05em] sm:text-[2.2rem]">
            Legajo RH
          </h1>
          <p class="mt-3 max-w-[12rem] text-[12px] leading-5 text-white/84">
          ...
          {(item, index) => {
            ...
            return (
              <button
                ...
                class={cn(
                  "flex items-center gap-3 rounded-3xl border px-3 py-2.5 text-left transition duration-200 md:min-h-[82px] xl:min-h-0",
                  props.route === item.id
                    ? "border-white/22 bg-white/16 shadow-card"
                    : "border-transparent bg-white/10 text-white/88 hover:bg-white/14"
                )}
              >
                <span class="grid h-9 w-9 place-items-center rounded-2xl bg-white/14 text-white">
                  <Icon size={18} />
                </span>
