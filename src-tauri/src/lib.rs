...
#[tauri::command]
fn get_legajo_detail(legajo_id: i64, state: tauri::State<'_, AppState>) -> Result<LegajoDetail, String> {
  let connection = open_connection(&state)?;
...
fn add_attachment(
  ...
  let Some(source_path) = FileDialog::new()
    .add_filter("Documentos", &["pdf", "png", "jpg", "jpeg", "webp", "tif", "tiff"])
    .pick_file()
  else {
...
#[tauri::command]
fn open_attachment(attachment_id: i64, state: tauri::State<'_, AppState>) -> Result<OpenResponse, String> {
  let connection = open_connection(&state)?;
...
fn list_legajos_internal(connection: &Connection, filters: &Filters) -> Result<Vec<LegajoSummary>, String> {
...
fn get_legajo_detail_internal(connection: &Connection, legajo_id: i64) -> Result<LegajoDetail> {
  ...
  let mut attachment_statement =
    connection.prepare("SELECT * FROM adjuntos WHERE legajo_id = ? ORDER BY fecha_carga DESC")?;
...
fn save_legajo_internal(connection: &mut Connection, payload: SaveLegajoPayload) -> Result<LegajoDetail> {
  ...
  let legajo_id = if let Some(current_record) = current {
    ...
    record_movement(&transaction, inserted_id, "creacion", "Creacion inicial del legajo.")?;
    inserted_id
  };
...
fn import_legajos_internal(
  ...
  for (index, row) in rows.into_iter().enumerate() {
    if row.numero_legajo.trim().is_empty() {
      skipped += 1;
      continue;
    }
    ...
    let existing_id: Option<i64> = connection
      ...
      .query_row(params![row.numero_legajo.trim()], |r| r.get(0))
      .optional()?;
    ...
    match save_legajo_internal(connection, payload) {
      ...
      Err(error) => errors.push(format!("Fila {}: {}", index + 2, error_message(error))),
    }
  }
...
fn import_legajos_workbook(file_path: &Path) -> Result<Vec<SaveLegajoPayload>> {
  let mut workbook = open_workbook_auto(file_path)?;
  let first_sheet = workbook
    .sheet_names()
    .first()
    .cloned()
    .ok_or_else(|| anyhow!("El archivo no contiene hojas."))?;
  ...
  let headers = header_row
    .iter()
    .map(|cell| map_header_key(&cell.to_string()))
    .collect::<Vec<_>>();

  let mut result = Vec::new();
  for row in rows {
    let mut payload = SaveLegajoPayload::default();
    for (index, cell) in row.iter().enumerate() {
      let key = headers.get(index).cloned().unwrap_or_default();
      let value = cell.to_string().trim().to_string();
      assign_import_value(&mut payload, &key, value);
    }
    if !payload.estado_legajo.trim().is_empty() {
...
fn assign_import_value(payload: &mut SaveLegajoPayload, key: &str, value: String) {
  match key {
    "numero_legajo" => payload.numero_legajo = value,
    "apellidos_nombres" => payload.apellidos_nombres = value,
    "dni" => payload.dni = value,
    "organo_unidad" => payload.organo_unidad = value,
    "cargo_puesto" => payload.cargo_puesto = value,
    "regimen_laboral" => payload.regimen_laboral = value,
    "fecha_vinculacion" => payload.fecha_vinculacion = normalize_date_input(&value),
    "estado_legajo" => payload.estado_legajo = value,
    "ubicacion_legajo" => payload.ubicacion_legajo = value,
    "observaciones" => payload.observaciones = value,
    _ => {}
  }
}

fn map_header_key(raw: &str) -> String {
  match normalize_header(raw).as_str() {
    "numerolegajo" | "numerodelegajo" => "numero_legajo".to_string(),
    "apellidosynombres"
    | "apellidosynombresdelaservidoracivil"
    | "apellidosynombresdelservidorcivil" => "apellidos_nombres".to_string(),
    "numerodedocumentodeidentidad" | "dni" => "dni".to_string(),
    "organoounidadorganica" => "organo_unidad".to_string(),
    "nombredelcargoestructuralyopuesto" | "cargoestructuralyopuesto" => "cargo_puesto".to_string(),
    "regimenlaboral" => "regimen_laboral".to_string(),
    "fechadevinculacion" => "fecha_vinculacion".to_string(),
    "estadodellegajo" | "estadodellegajoactivoopasivo" => "estado_legajo".to_string(),
    "ubicaciondellegajo" => "ubicacion_legajo".to_string(),
...
fn export_legajos_pdf(file_path: &Path, rows: &[LegajoSummary], filters: &Filters) -> Result<()> {
  ...
  let document =
    PdfDocument::from_html(&html, &images, &fonts, &options, &mut warnings).map_err(|e| anyhow!(e))?;
...
fn sanitize_legajo_payload(mut payload: SaveLegajoPayload) -> Result<SaveLegajoPayload> {
  ...
  payload.origen_registro = Some(
    match payload.origen_registro.unwrap_or_else(|| "manual".to_string()).as_str() {
...
fn build_change_detail(previous: &LegajoRecord, next: &SaveLegajoPayload) -> String {
  let pairs = [
    ("Numero de legajo", previous.numero_legajo.as_str(), next.numero_legajo.as_str()),
    (
      "Apellidos y nombres",
      previous.apellidos_nombres.as_str(),
      next.apellidos_nombres.as_str(),
    ),
    ("Documento de identidad", previous.dni.as_str(), next.dni.as_str()),
    ("Organo o unidad organica", previous.organo_unidad.as_str(), next.organo_unidad.as_str()),
    ("Cargo o puesto", previous.cargo_puesto.as_str(), next.cargo_puesto.as_str()),
    ("Regimen laboral", previous.regimen_laboral.as_str(), next.regimen_laboral.as_str()),
    (
      "Fecha de vinculacion",
      previous.fecha_vinculacion.as_str(),
      next.fecha_vinculacion.as_str(),
    ),
    ("Estado del legajo", previous.estado_legajo.as_str(), next.estado_legajo.as_str()),
    (
      "Ubicacion del legajo",
      previous.ubicacion_legajo.as_str(),
      next.ubicacion_legajo.as_str(),
    ),
    ("Observaciones", previous.observaciones.as_str(), next.observaciones.as_str()),
  ];
  ...
  let changes = pairs
    ...
    .filter_map(|(label, previous_value, next_value)| {
      if previous_value != next_value {
        Some(format!(r#"{}: "{}" -> "{}""#, label, previous_value, next_value))
      } else {
        None
      }
    })
...
fn record_movement(connection: &Connection, legajo_id: i64, kind: &str, detail: &str) -> Result<()> {
  connection.execute(
